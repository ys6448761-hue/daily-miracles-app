...
## ⚡ 지금 (한 줄 요약)
소원꿈터 MVP 2주 집중 개발 중 (D-Day: 2026-03-12)
...
🟢 완료: 4인 이하 자동 일정 생성 + PDF (AI + Puppeteer)
🟢 완료: 일정 빌더 프론트엔드 (3단계 폼 + 결과 페이지 + 메인 배너)
🟢 완료: Day별 고유 일정 템플릿 (4일차까지 각각 다른 코스)
🟢 완료: P3-1 5인 이상 단체 전환 브릿지 (단체 견적 CTA + source/must_go 저장)
...
### 2026-01-13: P3-1 5인 이상 단체 전환 브릿지

| 작업 | 상태 | 산출물 |
|------|------|--------|
| 단체 전환 모달 UI (pax≥5) | ✅ | `itinerary-builder.html` 그라데이션 카드 |
| 연락처 입력 필드 (이름, 전화번호) | ✅ | group-bridge 폼 |
| `/api/v2/quote/wix` 호출 | ✅ | source='itinerary_builder' |
| GA4 이벤트 4종 | ✅ | itinerary_group_convert_*, itinerary_builder_submit, itinerary_pdf_download |
| T1~T4 테스트 통과 | ✅ | 프로덕션 검증 완료 |

### P3-1 데이터 매핑

| 빌더 필드 | wix API 필드 | 저장 위치 |
|----------|--------------|----------|
| state.pax | pax | guest_count |
| startDate | travel_date | travel_date |
| state.mustVisit | must_go | notes ([필수방문] 태그) |
| state.avoid | avoid | notes ([회피] 태그) |
| 'itinerary_builder' | source | source 컬럼 |
| state.partyType | party_type | notes ([구성] 태그) |
| state.transport | transport | notes ([이동] 태그) |
| state.tempo | tempo | notes ([템포] 태그) |
| nights | nights | notes ([숙박] 태그) |

### P3-1 테스트 결과

| 테스트 | 시나리오 | 결과 | Quote ID |
|--------|----------|------|----------|
| T1 | pax=4 → 일정 생성 | ✅ | ITN-20260113-9CDD |
...
| 작업 | 상태 | 산출물 |
|------|------|--------|
| 일정 빌더 입력 폼 (3단계) | ✅ | `public/itinerary-builder.html` |
| 일정 결과 페이지 | ✅ | `public/itinerary-result.html` |
| 메인 페이지 배너 추가 | ✅ | `public/index.html` |
| 푸터 링크 추가 | ✅ | "여행 일정" 링크 |
| 전체 플로우 테스트 | ✅ | 메인→빌더→생성→결과→PDF |

### 일정 빌더 3단계 폼

| 단계 | 입력 항목 |
|------|----------|
| Step 1 | 날짜, 인원 (1-4인), 일행 유형 (커플/친구/가족/혼자) |
| Step 2 | 숙박 (당일~3박4일), 교통, 템포, 여행 취향 슬라이더 |
| Step 3 | 필수 방문지 태그, 회피 조건 태그, 추가 요청 |
...
| 기능 | 설명 |
|------|------|
| 요약 카드 | 기간/인원/교통/템포 표시 |
| PDF 다운로드 | 일정표 PDF 즉시 다운로드 |
| 타임라인 | 일차별 스팟 카드 표시 |
| AI 추천 | 맛집, 여행 팁 표시 |
| 수정 요청 | 무료 1회 간단 수정 모달 |

### 전체 플로우 테스트 결과 (2026-01-13)

| 단계 | 항목 | 결과 |
|------|------|------|
| 1 | 메인 페이지 배너 | ✅ 2개 링크 |
| 2 | 빌더 페이지 | ✅ 200 OK (33KB) |
| 3 | 옵션 API | ✅ success |
| 4 | 일정 생성 | ✅ ITN-20260113-6E2A |
| 5 | 결과 페이지 | ✅ 200 OK |
| 6 | PDF 다운로드 | ✅ 120KB |

### Day별 고유 일정 템플릿 (2026-01-13 수정)

| Day | 테마 | 코스 |
|-----|------|------|
| **Day 1** | 여수의 낭만 시작 | 오동도 → 중앙시장 → 해상케이블카 → 낭만포차 |
| **Day 2** | 여수 동쪽 탐험 | 향일암 → 돌산 갯장어거리 → 금오도 비렁길 → 돌산공원 |
| **Day 3** | 문화와 미식의 날 | 진남관 → 서대회타운 → 아쿠아플라넷 → 엑스포 해양공원 |
| **Day 4** | 숨은 보석 발견 | 거문도/백도 → 거문도 해녀촌 → 해양레일바이크 → 웅천친수공원 |

### 프로덕션 테스트 (루미 URL 수정 후)

| 항목 | 결과 |
|------|------|
| 올바른 URL | `https://daily-miracles-app.onrender.com` |
| 잘못된 URL | ~~`https://daily-miracles.onrender.com`~~ |
| 테스트 일정 ID | `ITN-20260113-B092` (2박3일 커플) |
| Day 1 | ✅ 오동도 → 중앙시장 → 케이블카 → 낭만포차 |
| Day 2 | ✅ 향일암 → 갯장어거리 → 금오도 → 돌산공원 |
| Day 3 | ✅ 진남관 → 서대회타운 → 아쿠아플라넷 → 엑스포 |

### 커밋 이력

```
e7aa698 - fix(quote): DB INSERT에 source, notes, env 컬럼 추가
57f4e33 - fix(quote): source/must_go/avoid 필드 저장 지원 (P3-1 T4)
e39ca2e - feat(itinerary): P3-1 5인 이상 단체 전환 브릿지 구현
a832979 - docs: AURORA_STATUS.md 업데이트 (Day별 고유 일정)
bcf53e1 - fix(itinerary): Day별 고유 일정 템플릿 적용 (4일차까지)
a4ffa5f - feat(itinerary): 일정 빌더 프론트엔드 페이지 추가
fc95bf0 - feat(index): 메인 페이지에 일정 빌더 링크 추가
```
...
| 2026-01-13 | Code | P2-2 정산서/수수료-only PDF 자동 생성 (commission 모드) |
