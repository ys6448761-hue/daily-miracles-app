 ...
 * Usage: DATABASE_URL=... node scripts/run-migration-038.js
 */
...
async function run() {
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, '../database/migrations/038_voyage_logs_source.sql'),
    'utf8'
  );

  console.log('🚀 Migration 038 시작...');
  try {
    await pool.query(sql);
    console.log('✅ dt_voyage_logs.source 컬럼 추가 완료');
