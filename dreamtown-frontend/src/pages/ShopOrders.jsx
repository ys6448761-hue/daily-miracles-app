...
export default function ShopOrders() {
  ...
  const starGrowth   = state?.starGrowth;
    ...
    <div style={S.page}>
      ...
      {newOrderId && (
        <motion.div style={S.successBanner} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {starGrowth && (
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(52,211,153,0.2)' }}>
              <div style={{ fontSize: 13, color: '#E8E4F0', marginBottom: 8, lineHeight: 1.6 }}>
                ✨ 선물을 보낸 당신의 별이<br />조금 더 밝아졌습니다
              </div>
              <button
                style={{ background: 'none', border: '1px solid rgba(52,211,153,0.4)', borderRadius: 8, padding: '6px 14px', color: '#34d399', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}
                onClick={() => nav('/my-star')}
              >
                내 별 확인하기 →
              </button>
