...
기적쇼츠 =
  소원그림 1P (발견)
  + 배우 소원이 별빛항로 7개 항로 × 5초  (= 35초, 항로별 5초 클립 7개 연결)
  + 소원그림 2P (응답)
  + 엔딩
```
...
| 배우 소원이 별빛항로 7개 항로 × 5초 | "배우 소원이"라는 별도 소재(영상/캐릭터)의 정의, 7개 항로 각각의 5초 클립 제작 방식 | **완전 미정 — 소재 자체가 코드베이스에 없음** |
...
2. 7개 항로가 무엇을 지칭하는지(SSOT-ROUTE-001 등 기존 항로 정의 문서와의 연결 여부) 확인.
