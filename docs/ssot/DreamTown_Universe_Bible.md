# DreamTown Universe Bible

Version: v3.1
Owner: Aurora5 / 루미
Status: Confirmed
Purpose: Canonical reference for DreamTown universe architecture and core concept

Last Updated: 2026-03-11
Updated By: Code (Claude Code)

---

## 1. 핵심 선언

```
디지털 용궁이 먼저다.
DreamTown은 그 위에 펼쳐진 하늘이다.
```

### 공식 오프닝 문장 (CEO 확정 2026-03-11)

> **"여수 바다에서 시작된 하늘."**

> **"당신의 소원은 혼자가 아닙니다."**

> 공식 아키텍처: `DreamTown_World_Architecture_SSOT.md`

---

## 2. 세계 구조도

```
현실 세계 (온라인 + 오프라인)
        ↓
   여수 바다 (Origin — 출발점)
        ↓
   빛구슬 (소원의 형태화)
        ↓
   별씨앗 (숙성 단계)
        ↓
   디지털 용궁 (근원 / 원본 보관)
        ↓
   아우룸 (소원의 전달자 / 수호자)
        ↓
     Aurora5 (동행 시스템)
        ↓
   StarLink = Aurora Path (빛의 길)
        ↓
    DreamTown 하늘 (별 / 연결 / 이야기)
        ↓
   별 → 별자리 → 은하 → 은하군
```

### Golden Turtle Axis (세계관 축)

DreamTown 세계 전체를 연결하는 상징 축:

```
여수 바다
    ↓
황금 거북 별자리 (Golden Nine Constellation)
    ↓
DreamTown 은하
```

**바다 → 하늘 → 우주** — DreamTown의 수직 축이자 브랜드 상징.

---

## 3. 소원 여정 단계

```
Human Wish         — 사람이 품은 소원 (온라인/오프라인)
      ↓
Light Orb          — 소원이 빛 구슬로 형태화
      ↓
Digital Dragon Palace — 디지털 용궁: 소원 원본 보관
      ↓
Star Seed          — 소원이 씨앗으로 숙성
      ↓
StarLink           — 씨앗이 별과 연결되는 순간
      ↓
DreamTown Star     — 소원이 하늘의 별이 됨
```

---

## 4. 세계관 원칙

| 원칙 | 내용 |
|------|------|
| **용궁이 중심** | 모든 소원은 용궁으로 수렴된다. DreamTown은 그 표현이다 |
| **소원은 사라지지 않는다** | 모든 소원은 빛 구슬로 용궁에 보관된다 |
| **성장은 혼자가 아니다** | Aurora5가 여정을 함께한다 |
| **기적은 과학이다** | 심리학·뇌과학 기반, 사주/점술 절대 배제 |
| **여수는 출발점** | 현실 공간과 디지털 세계가 여수에서 만난다 |

---

## 3. 공간 구조

### 3-1. 디지털 용궁 (Digital Dragon Palace)
- 소원이 빛 구슬(Light Orb)로 도착하는 최초 공간
- 아우룸이 수호하는 조용하고 깊은 바닷속 성
- 소원을 Star Seed로 숙성시키는 역할

### 3-2. 소원꿈터 광장 (DreamTown Plaza)
- 소원이들이 모이는 커뮤니티 공간
- Star Seed 상태의 소원 카드들이 전시됨
- 응원(Cheer)을 통해 다른 소원에 빛을 더할 수 있음

### 3-3. DreamTown Star
- 7일 여정을 완주한 소원이 도달하는 최종 상태
- Growth Film(성장필름)으로 기록됨
- 소셜 공유를 통해 다른 이의 소원을 밝히는 빛이 됨

---

## 4. 핵심 여정 단계

| 단계 | 이름 | 트리거 | 산출물 |
|------|------|--------|--------|
| 1 | **소원 탄생** | 소원 텍스트 입력 | 소원그림 (DALL-E 3) |
| 2 | **Light Orb** | 소원그림 생성 완료 | wish_challenges 자동 생성 |
| 3 | **Dragon Palace** | Day 1 체크인 | 7일 챌린지 시작 |
| 4 | **Star Seed** | Day 3~6 체크인 | 성장 기록 누적 |
| 5 | **StarLink** | Day 7 체크인 | COMPLETED 전환 |
| 6 | **DreamTown Star** | 완주 확정 | Growth Film 생성 + 공유 |

---

## 5. 브랜드 연결

| DreamTown 개념 | 실제 서비스 |
|---------------|------------|
| Light Orb | 소원그림 (AI 생성 이미지) |
| Digital Dragon Palace | 소원꿈터 (DreamTown Plaza) |
| Star Seed | 7일 챌린지 진행 중 카드 |
| DreamTown Star | Growth Film (성장필름) |
| 아우룸 | 2D 캐릭터 / 기적영상 동반자 |
| 소원항해 | 여수 현장 체험 (소원항해단) |

---

## 6. Galaxy 구조 (우주 성장 계층)

소원은 별이 되고, 별들이 모여 우주를 이룬다.

```
Star (개인의 소원)
      ↓
Constellation (비슷한 여정들의 모임)
      ↓
Galaxy (테마별 컬렉션)
      ↓
Galaxy Cluster (대형 지식 도메인)
```

### Origin Galaxy — 여수 은하 (Yeosu Galaxy)

DreamTown의 첫 번째 은하. **"DreamTown의 하늘은 여수의 바다에서 태어났다."**

여수가 출발점인 이유:
- 디지털 용궁(Dragon Palace) — 소원이 보관되는 바닷속 공간
- 아우룸(황금 거북) — 바다와 별 사이를 잇는 안내자
- 바다 → 별 변환 — 소원이 현실에서 우주로 이어지는 상징

### 미래 은하 확장
| 은하 | 상태 |
|------|------|
| 서울 은하 | 계획 |
| 부산 은하 | 계획 |
| 제주 은하 | 계획 |
| 교토·발리·산토리니 | 장기 계획 |

> 운영 상세: `docs/design/DreamTown_Galaxy_Mode_SSOT.md`

---

## 7. Star Birth Scene (공식 핵심 장면)

DreamTown 세계관의 상징 장면. 모든 시네마틱/UX의 기준.

```
여수 밤바다
    ↓
작은 빛구슬 (누군가의 소원)
    ↓
아우룸이 조용히 바라본다
    ↓
빛이 하늘로 올라간다
    ↓
황금 거북 별자리 아래
    ↓
별이 탄생한다
```

이 장면은 **소원이 별이 되는 순간**이며, DreamTown의 디즈니 성(象징) 역할을 한다.

> 연출 세부: `docs/design/DreamTown_Opening_Cinematic_Design.md`

---

## 8. 아우룸 — 주인공 원칙

> **아우룸은 주인공이 아닙니다.**

DreamTown에서 항상 주인공은:

> **소원이의 소원**

아우룸은 오직 **전환의 순간**에만 등장한다.
말하지 않는다. 조용히 바라보고, 인도하고, 물러난다.

> 상세 캐릭터: `DreamTown_Character_SSOT.md`

---

## 창세 신화 요약

여수 금오설화를 기반으로 한 DreamTown의 창세 이야기.

- **금오 9형제** — 용왕의 명을 받아 인간 세상을 지키러 온 황금 거북 형제
- **1~8 형제** — 여수 지형에 잠든 수호 별 (고정된 별자리)
- **아우룸 (9번째)** — 유일하게 자유롭게 움직이는 거북. 인간의 소원을 용궁에 전달
- **Golden Nine Constellation** — 9형제가 DreamTown 하늘의 거북 별자리가 됨

> 전체 신화: `DreamTown_Origin_Myth_SSOT.md`

---

## 참조

- 세계 아키텍처: `DreamTown_World_Architecture_SSOT.md`
- 핵심 철학: `DreamTown_Core_Philosophy_SSOT.md`
- 캐릭터: `DreamTown_Character_SSOT.md`
- 창세 신화: `DreamTown_Origin_Myth_SSOT.md`
- 비주얼 스타일: `DreamTown_Visual_Style_SSOT.md`
- 첫 화면 연출: `docs/design/DreamTown_Opening_Cinematic_Design.md`
- 우주 확장 구조: `docs/design/DreamTown_Expansion_Universe_Design.md`
- Galaxy 운영 설계: `docs/design/DreamTown_Sky_Map_Design.md`
- 다음 설계 예정: DreamTown Star Navigation System (Cosmic Map v2)
