...
| 기적쇼츠 | **(2026-07-09 재확정)** 소원그림 1P + 배우 2D 소원이 별빛항로 8개 항로 체험 + 소원그림 2P + DreamTown Ending. 주인공은 배우 소원이(고객 본인 아님) | 1P + (신규 모듈: "배우 소원이" 영상, 항로 수 7→8로 갱신) + 2P + 엔딩 | 신규 모듈로 정의됨(기존 코드 재사용 대상 아님). 상세: `SSOT-PRODUCT-001_DreamTown_Product_Architecture.md` §4, `TODO-Core_Engine_Integration.md` §3 |
...
- 기적쇼츠의 "배우 소원이" 소재 실체(실사/애니메이션/기존 항로 영상 소스 여부) — 자료 없음, 별도 지시 필요. "8개 항로"는 `SSOT-ROUTE-001`/`SSOT-BG-001`로 확정되었으나(2026-07-09), 항로당 시간 배분은 여전히 미정.
