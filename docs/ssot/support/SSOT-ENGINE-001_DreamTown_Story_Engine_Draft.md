...
| 기적쇼츠 | **(확정, 2026-07-09)** 소원그림 1P + 배우 소원이 별빛항로 7개 항로×5초(=35초) + 소원그림 2P + 엔딩 | 1P + (신규 모듈: "배우 소원이" 영상 7클립) + 2P + 엔딩 | 신규 모듈로 정의됨(기존 코드 재사용 대상 아님). 상세: `TODO-Core_Engine_Integration.md` §3 |
...
- 기적쇼츠의 "배우 소원이" 소재 실체(실사/애니메이션/기존 항로 영상 소스 여부)와 "7개 항로" 정의(기존 항로 문서와의 연결 여부) — 자료 없음, 별도 지시 필요. 구조 자체는 2026-07-09 확정됨(위 §4 참조).
