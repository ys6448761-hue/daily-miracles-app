...
- 📊 추적/분석: "이 이벤트를 로깅하면 인사이트를 얻습니다"

---

### 📋 출력 형식 (필수 준수)

모든 작업 완료 보고 시 아래 형식 사용:
...
| 새 UI 컴포넌트 | 모바일 반응형, 로딩 상태, 에러 상태, 접근성 |
| 새 DB 작업 | 트랜잭션, 인덱스, 에러 롤백 |
| 새 메시지/콘텐츠 | 역순 전략 적용, 금지어 체크, A/B 버전 |
| 파일 업로드/다운로드 | 용량 제한, 형식 검증, 실패 처리 |
| 결제/금액 관련 | 검증 로직, 환불 처리, 영수증, 로깅 |
| 알림 발송 | 발송 실패 처리, 재시도, 로깅 |
